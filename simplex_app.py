...
# Importe a classe ydata e a função optimize_portfolio do seu arquivo return_target_simplex.py
from return_target_simplex import ydata, portfolio_volatility, portfolio_return, optimize_portfolio_simplex, optimize_portfolio
...
if st.button('Otimizar Portfólio'):
    if tickers_input:
        ...
        pesos, desempenho = optimize_portfolio(tickers, data_inicio.strftime("%Y-%m-%d"), 
                                               ...
                                               retorno_alvo, alocacao_maxima)
        
        if isinstance(desempenho, tuple):
            ...
            st.error(f"Erro na otimização: {desempenho}")
    else:
...
st.sidebar.write('Este aplicativo usa o algoritmo simplex de otimização linear.')